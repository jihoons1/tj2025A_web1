...
/*
    ...
    4. 
        ...
        [생성]
    5. 생성된 앱 클릭 -> 대시보드 접속
    6. 사이드바메뉴 -> [앱]->[일반] -> 중간 스크롤 쯤에 '앱 키' 확인 가능.
        * JS 카카오지도 사용시에는 'JavaScript 키'  사용한다.
    7. '플랫폼' Web 주소 등록 -> [Web 플랫폼 등록]
        http://localhost:8080
        http://192.168.40.189:8080
        * 추후에 배포단계에서는  실제 존재하는 도메인 또는 클라우드 ip를 등록한다.
    8. 사이드바메뉴 -> [카카오맵] 
        사용 설정 -> 활성화 * 계정마다 카카오지도 1개 활성화

    [ kakao 지도 ]
     1. https://apis.map.kako.com/
     2. [web] 클릭 -> 횐쪽 메뉴 -> [가이드] -> 시작

*/
// [1] 간단한 주소 출력
const kakakomap1 = async () => {
// (1) 지도를 담을 영역의 DOM 레퍼런스
var container = document.getElementById('map');
// (2) 지도를 생성할 때 필요한 기본 옵션
var options = { 
	center: new kakao.maps.LatLng(37.4905335508, 126.7244907601), //지도의 중심좌표. // 지도 중심좌표ㅕ 부평역 37.4905335508 , 126.7244907601
...
const kakaoMap4 = async() => {
    // 1. 지도를 표시할 div  
    var mapContainer = document.getElementById('map'), mapOption = {  center: new kakao.maps.LatLng(33.450701, 126.570667), level: 3  }; // 지도의 확대 레벨
    // 2. 지도를 생성합니다
    var map = new kakao.maps.Map(mapContainer, mapOption); 
    ...
    // 4. 마커 이미지의 이미지 주소입니다 // static -> img 폴더내 이밎 경로
