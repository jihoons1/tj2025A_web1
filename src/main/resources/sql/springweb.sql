...
select * from post;
# 2. 카테고리가 일치한 게시물 조회
select * from prost where cno = 1;
# 3. 정렬 , order by 필드명 asc오름차순 / desc 내림차순
select * from post where cno = 1 order by pno desc;
# 4. 페이징 처리 : 1페이지 limit 0 , 5 / 2페이지 limit 5 ,5 / 3페이지 limit 10 , 5
# 5. limit 시작인덱스 , 개수
select * from post p inner join member m on p.mno = m.mno;
# 6. 4번과 5번 합치기
select * 
