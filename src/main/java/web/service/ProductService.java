...
public class ProductService {
    ...
    public int createProduct(ProductDto dto) {
        return productDao.createProduct(dto); // DB에 제품 저장
    }
    ...
    public List<ProductDto> getAllProduct() {
        ...
        for (ProductDto productDto : productDtoList) { // 조회된 모든 제품dto를 반복
            List<String> images =
                    productDao.getProductImages(productDto.getPno()); // 특정한 제품의 pno를 이용하여 모든 제품이미지
    ...
    public ProductDto getProduct(int pno) {
        ...
        if (dto != null ){
            List<String> images = productDao.getProductImages(dto.getPno()); // [2-2] 메소드 호출
