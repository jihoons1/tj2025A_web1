...
@Service // 해당 클래스 스프링컨테이터 빈 등록
public class FileService {

    // [*] 업로드 경로,
    // 방법1) 프로젝트 src : 개발자가 코드를 작성하는 폴더
    // 방법2) 프로젝트내 build : 서버실행했을때 컴파일된 코드 즉] 서버(24시간) 의 갖는 실행 된 폴더

    // 1. 현재 프로젝트의 최상위 디렉토리(폴더) 경로 찾기
    private String baseDir = System.getProperty("user.dir");
    // 2. 방법2 처럼 개발자폴더가 아닌 실행된서버의 폴더로 업로드경로 지정하기 , *개발환경*에 따라 달라진다.
    private String uploadPath = baseDir + "/build/resources/main/static/upload/";

    // [1] 파일 업로드 : 스프링에서는 MultipartFile 인터페이지원( 대용량 바이트 조작 )
    public String fileUpload(MultipartFile multipartFile) {
        ...
        // - 업로드된 파일명에 _언더바가 존재하면 모두 -하이폰 변경 , 예] 짱구_사진.jsp -> 짱구-사진.jsp
        ...
        if (!pathFile.exists()) { // .exists() : 지정한 경로가 존재하면 true 존재하지않으면 false 반환 메소드
    ...
    public void fileDownload(String filename, HttpServletResponse response) {
        ...
        String donwloadPath = uploadPath + filename;
        ...
        try {
            //3-1 파일 입력스트림 객체 생성
            FileInputStream fin = new FileInputStream(donwloadPath);
            ...
            // 1. 파일의 void 제거 , split("기존문자") : 문자열내 기존문자 기준으로 문자열 자르기
            String oldFilename = filename.split("_")[1]; // UUID 파일명 ~하이폰 기준으로 잘라서 2번쨰 문자열 뜼
            // ** 2. 해당 형식 지정하기 ,다운로드 형식과 다운로드파일지정 , url은 한글 지원하징 ㅏㄶ는다 . URLEncoder.encode( 파일명 , "UTF-8") );
    ...
    public boolean fileDelete(String fileName) {
        ...
        if (file.exists()) { file.delete(); // .delete() : 지정한 경로상의 파일 삭제
            return true;
        }
        // 2. 아니면 flase
        return false;
