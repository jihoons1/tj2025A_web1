...
public class MemberDao extends Dao { // JDBC 연동 상속받기
    ...
    public boolean check(String type , String data ) {
        try {
            ...
            String sql = " select * from member where " + type + " = ?"; // 중복 검사
                // +  + 연산자 사용시 띄어쓰기 확인
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, data);
            ResultSet rs = ps.executeQuery();
    ...
    // [8] 회원탈퇴
    public boolean delete(int mno , String oldpwd){ // old(삭제하기전(확인용)
        try{
    ...
    // [9] 아이디 찾기
    public String infoid( Map<String ,String > map){
        try{
    ...
    public boolean infopass( Map<String , String > map){
        try{
            ...
            System.out.println(e+"비밀번호 찾기 문제");
        }return false;
